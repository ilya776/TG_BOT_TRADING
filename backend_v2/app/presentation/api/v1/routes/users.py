...
User profile, settings, API keys, and statistics management.
"""
...
async def add_api_key(
    ...
    if existing:
        raise HTTPException(
            ...
            detail=f"Active API key for {request.exchange.value} already exists",
        )
...
async def get_user_stats(
    ...
    win_rate = (
        (Decimal(winning_trades) / Decimal(total_closed) * 100)
        if total_closed > 0
        else Decimal("0")
    )
