...
Trade history, positions, portfolio management.
"""
...
async def close_position(
    ...
    Note: Full implementation requires exchange adapters.
    This updates the position status in the database.
    """
    result = await db.execute(
        select(Position).where(
            Position.id == position_id,
    ...
    # TODO: Implement actual position close using exchange adapters
    # This requires:
    # 1. Get user's API key for the exchange
    # 2. Initialize exchange executor
    # 3. Execute market sell order
    # 4. Update position with actual execution results

    # For now, mark position as closed with manual reason
    now = datetime.utcnow()
    ...
    # Use unrealized PnL as realized PnL
    ...
    logger.info(f"Position {position_id} closed via API. PnL: {position.realized_pnl}")
